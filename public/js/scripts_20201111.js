...
$('.add').click(function(){
  ...
  i++;
  $('#dynamic_field'+add_id).append('</br><tr id="comments'+i+'"><td><input type="text" name="comments" class="form-control comments" placeholder="Comments Here"></td><td><button type="button" name="remove_comment" id="'+i+'" class="btn btn-danger btn_remove">X</button></td><td><button type="submit" id="'+i+'" class="btn btn-info btn_save">Save</button></td></tr>');
});
      ...
      success: function(data){
        ...
        for(var i=0;i<data.comments.length;i++)
        {
          if(inc_compare == i){
            ...
            $('#dynamic_field'+table_id).append('<tr class="dynamic_comments"><td><input type="text" class="form-control comments" value="'+data.comments[i]+'" reaonly></td></tr>');
          }
