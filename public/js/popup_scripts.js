...
$('.reply_get').click(function(){
  ...
  html += '<td><button type="submit" id="reply_save'+j+'" data-attr="'+j+'" class="btn btn-info reply_save">&#10003;</button></td></tr>';
  html += '<tr><td><input type="file" class="image_reply filer btn btn-warning" name="image" id="image_reply'+j+'" style="width:199px;"></td></tr>';
...
$(document).on('click','.reply_save',function(){
  ...
  if(document.getElementById('image_reply'+button_id1).files.length == 0)
  {
        $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="image_replystatus" name="image_replystatus" id="image_replystatus" value="pass"></td></tr>');
        console.log('$("#image_status").val():::::',$("#image_status").val());
  }
  else {
    $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="image_replystatus" name="image_replystatus" id="image_replystatus" value="passelse"></td></tr>');
    console.log('$("#image_status").val():::::',$("#image_status").val());
  }
  let comments_id_ajax = $("#comments_id_hidden"+mouse_id).val();
  let reply_ajax = $('#'+form_fetch).find("textarea[name=reply]").val();
  let username_comments1_ajax = $('#'+form_fetch).find("input[name=username_comments]").val();
  let email_comments1_ajax = $('#'+form_fetch).find("input[name=email_comments1]").val();

  if(comments_id_ajax != "")
  {
    $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="comments_id_ajax" name="comments_id_ajax" id="comments_id_ajax" value="'+comments_id_ajax+'"></td></tr>');
    console.log('$("#image_status").val():::::',comments_id_ajax);
  }
    if(reply_ajax != "")
    {
      $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="reply_ajax" name="reply_ajax" id="reply_ajax" value="'+reply_ajax+'"></td></tr>');
      console.log('$("#image_status").val():::::',reply_ajax);
    }
      if(username_comments1_ajax != "")
      {
        $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="username_comments1_ajax" name="username_comments1_ajax" id="username_comments1_ajax" value="'+username_comments1_ajax+'"></td></tr>');
        console.log('$("#image_status").val():::::',username_comments1_ajax);
      }
        if(email_comments1_ajax != "")
        {
          $('#tr'+mouse_id).append('<tr><td><input type="hidden" class="email_comments1_ajax" name="email_comments1_ajax" id="email_comments1_ajax" value="'+email_comments1_ajax+'"></td></tr>');
          console.log('$("#image_status").val():::::',email_comments1_ajax);
        }
